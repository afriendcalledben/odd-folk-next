    ...
    <header className="bg-brand-blue sticky top-0 z-50 border-b border-white/10">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-24 sm:h-28">
          ...
          <div className="flex-shrink-0 cursor-pointer py-2 group" onClick={() => handleNavigate('/')}>
                ...
                <div className="relative" ref={dropdownRef}>
                    ...
                    {isDropdownOpen && (
                        <div className="absolute right-0 mt-3 w-56 bg-white rounded-xl shadow-xl border border-brand-grey/20 overflow-hidden py-2">
                            <button onClick={() => handleNavigate('/dashboard?tab=listings')} className="w-full text-left px-4 py-2 text-base text-brand-burgundy hover:bg-brand-grey/5 hover:text-brand-orange transition-colors">📦 Listings</button>
                            <button onClick={() => handleNavigate('/dashboard?tab=bookings')} className="w-full text-left px-4 py-2 text-base text-brand-burgundy hover:bg-brand-grey/5 hover:text-brand-orange transition-colors">📅 Bookings</button>
                            <button onClick={() => handleNavigate('/dashboard?tab=profile')} className="w-full text-left px-4 py-2 text-base text-brand-burgundy hover:bg-brand-grey/5 hover:text-brand-orange transition-colors">👤 Profile Settings</button>
                        </div>
                    )}
                </div>
             ) : (
