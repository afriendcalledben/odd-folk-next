...
const Login: React.FC<{ initialMode?: 'login' | 'signup' }> = ({ initialMode = 'login' }) => {
    ...
    <div className="min-h-screen bg-brand-white flex items-center justify-center p-4 relative">
      ...
      <div className="bg-brand-blue w-full max-w-[480px] rounded-3xl shadow-2xl relative z-10 p-8 sm:p-12 border border-brand-white/10 animate-fade-in text-brand-white">
        ...
        <div className="text-center mb-10">
          <div className="w-24 mx-auto mb-6 cursor-pointer filter brightness-0 invert" onClick={() => router.push('/')}>
             <Logo />
          </div>
          <h1 className="font-heading text-3xl text-brand-white mb-2">
            {isLogin ? 'Welcome back, neighbour' : 'Join the folk'}
          </h1>
          <p className="font-body text-brand-white/70">
            {isLogin ? 'Log in to find your next treasure' : 'Create an account to start sharing'}
          </p>
        </div>

        <div className="space-y-3 mb-8">
          <button className="w-full flex items-center justify-center px-4 py-3.5 border border-brand-white/20 rounded-xl font-body font-medium text-brand-white hover:bg-white/5 hover:border-brand-white/40 transition-all relative group bg-brand-blue shadow-sm">
