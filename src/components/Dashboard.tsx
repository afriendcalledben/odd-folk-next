...
const BookingCard: React.FC<BookingCardProps> = ({ booking, isLister, onStatusChange }) => (
...
const Dashboard: React.FC<DashboardProps> = ({ user, activeTab = 'listings' }) => {
    ...
    const renderSidebar = () => {
        const tabs = [
            { id: 'listings', label: 'My Collection', icon: '📦' },
            { id: 'bookings', label: 'Experiences', icon: '📅' },
            { id: 'favorites', label: 'Favourites', icon: '💙' },
            { id: 'profile', label: 'My Profile', icon: '👤' },
            { id: 'locations', label: 'Collection Points', icon: '📍' },
    ...
    const renderContent = () => {
        ...
        switch (currentTab) {
            case 'listings':
                return (
                    <div className="space-y-6">
                        <h2 className="font-heading text-3xl text-brand-blue mb-8">Your Collection</h2>
            ...
            case 'bookings':
                return (
                    <>
                        <h2 className="font-heading text-3xl text-brand-blue mb-8">Your Experiences</h2>
                        {bookings.filter(b => b.hirerId === currentUserId).map(b => (
                            <BookingCard key={b.id} booking={b} isLister={false} onStatusChange={handleStatusChange} />
                        ))}
                    </>
                );
